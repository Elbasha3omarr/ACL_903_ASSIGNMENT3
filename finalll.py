...
currentEmbedModel = "paraphrase-MiniLM-L6-v2"
...
class FPLInputPreprocessor:
    ...
    def __init__(self, embedding_model_name: str = currentEmbedModel, team_names_from_kg: Optional[List[str]] = None, player_names_from_kg: Optional[List[str]] = None):
        ...
        self.intent_examples = {
            "player_performance": [
                ...
                "Total points for Erling Haaland last season",
                "Assists by Kevin De Bruyne in 2022-23",
                "What are Bukayo Saka stats this season?",
                "How did Harry Kane perform?",
                "Harry Kane goals and assists this year",
                "Show me Harry Kane assists"
            ],
            "player_comparison": [
                "Compare Erling Haaland and Harry Kane",
                "Who is better Salah or De Bruyne?",
                "Who is better Salah versus Erling Haaland?",
                "Haaland vs Harry Kane performance comparison",
                "Salah vs Son performance",
                "Haaland versus Harry Kane stats"
            ],
            "team_analysis": [
                "How did Arsenal perform this season?",
                "Liverpool defensive stats",
                "Man City clean sheets analysis",
                "Liverpool performance overview",
                "How strong is Chelsea defensively?",
                "Attacking stats for Chelsea",
                "Liverpool defence and attack stats"
            ],
            "fixture_query": [
                "When does Liverpool play against Man City?",
                "Next fixture for Arsenal",
                "Who does Chelsea play in GW15?",
                "Show me all fixtures in gameweek 15",
                "Upcoming matches",
                "Fixture schedule",
            ...
            "season_comparison": [
                "Compare Mohamed Salah performance in 2021-22 versus 2022-23",
                "Stats comparison between 2021-22 and 2022-23",
                "How did this player perform last season vs this season?",
                "Season over season comparison",
                "Performance across different seasons",
                "Last season versus this season"
            ],
            "position_analysis": [
                "Best midfielders in the league",
                "Who are the top scoring defenders?",
                "Top forwards by points",
                "Highest scoring goalkeepers",
                "Best players by position",
                "Position based rankings",
                "Best defenders this season",
                "Midfielders performance ranking"
            ],
            "recommendation": [
                "Who should I pick?",
                "Best captain choice this week",
                "Recommend a good midfielder",
                "Who should I transfer in?",
                "Is this player worth it?",
                "Good FPL picks right now",
                "Who should I buy?",
            ...
            "search_player": [
                ...
                "Find information about Mohamed Salah"
                "Get Bukayo Saka player information"
            ]
        }
    ...
    def classify_intent_embedding(self, query: str) -> Dict[str, float]:
        ...
        for intent, emb_matrix in self.intent_embeddings.items():
            sim = cos_sim(query_emb, emb_matrix)
            scores[intent] = sim.max().item()
    ...
    def classify_intent(self, query: str) -> Tuple[str, float]:
        ...
        for intent, patterns_with_weights in self.intent_patterns.items():
            score = 0.0
            total_possible_weighted_score = sum(w for _, w in patterns_with_weights)
            ...
            if total_possible_weighted_score > 0:
                intent_scores[intent] = min(1.0, score / total_possible_weighted_score)
            else:
        ...
        # 2) EMBEDDING SCORING
        # ==============================
        embedding_scores = self.classify_intent_embedding(query)
        ...
        for intent in intent_scores:
            regex_score = intent_scores[intent]
            embed_score = embedding_scores.get(intent, 0.0)

            if regex_score >= 0.45:
                final_scores[intent] = 0.5 * regex_score + 0.5 * embed_score
            else:
                final_scores[intent] = 0.15 * regex_score + 0.85 * embed_score

        best_intent = max(final_scores, key=final_scores.get)
        confidence = final_scores[best_intent]

        if confidence < 0.35:
            return "general_query", round(confidence, 3)
...
class FPLGraphRAGPreprocessor:
    ...
    def __init__(self, config_path: str, embedding_model: str = currentEmbedModel):
...
if test_all:
    ...
    preprocessor = FPLInputPreprocessor(embedding_model_name=currentEmbedModel, team_names_from_kg=team_names, player_names_from_kg=player_names)
    ...
    edge_cases = [
        "mohamed salah goals",
        "ARSENAL FIXTURES",
        "Salah vs Kane vs Haaland",
        "defenders with more than 5 goals and less than 2 yellow cards",
        "who scored the most?", 
        "gw10 fixtures",
        "2021-22 vs 2022-23 season comparison",
    ]
...
def run_processed_query(executor: Neo4jQueryExecutor, processed: ProcessedInput) -> List[Dict]:
    ...
    cypher_query = CypherQueryGenerator.get_query_template(processed)

    results = executor.execute_query(cypher_query, processed.cypher_params)
    return results

def run_processed_query2(executor: Neo4jQueryExecutor, processed: ProcessedInput) -> List[Dict]:
    processed.cypher_params = prepare_cypher_params(processed)
...
def llm_generate(prompt: str,
    ...
    try:
        ...
        print("Failed to find a response.\n")
        return None, None
...
def userQueryKG(userQuery : str, preprocessor : FPLInputPreprocessor, model="deepseek-ai/DeepSeek-V3", traceQuery=False):
    ...
    results, cypher_query = run_processed_query2(executor, processed)
    executor.close()
    ...
    if traceQuery:
        ...
        print(f"Entities: {processed.entities}")
        print(f"Context: {context_text[:20]}")
        print(f"Response: {response}")

    return response, tokens_used, processed, cypher_query

or_model1 = "openai/gpt-oss-20b:free"

hf_model1 = "deepseek-ai/DeepSeek-V3.2"
...
# query1 = "Which midfielder should I pick for my team?"
# print("-"*30)
# print("DeepSeek V3.2")
# print("-"*30)
# userQueryKG(query1, preprocessor, hf_model1, True)
# print("-"*30)
# print("Gemma-2-9b-it")
# print("-"*30)
# userQueryKG(query1, preprocessor, hf_model2, True)
# print("-"*30)
# print("Llama-3.3-70B-Instruct")
# print("-"*30)
# userQueryKG(query1, preprocessor, hf_model3, True)

MODELS = {
    "DeepSeekV3.2": hf_model1,
    "Gemma-2-9b-it": hf_model2,
    "Llama-3.3": hf_model3
}

MODELS0 = {
    "gpt-oss-20b": or_model1
}

EVAL_MODEL = hf_model2
...
def call_model(modelUsed, preprocessor : FPLInputPreprocessor, userQuery):
    start = time.time()

    response, tokens_used, _, _ = userQueryKG(userQuery, preprocessor, modelUsed)
...
# EVALUATION
# =================================================
if test_all and False:
    rows = []
    for model_name, model_id in MODELS.items():
        print(f"Evaluating {model_name}")
        for case in test_cases:
            print(f"Current query execution: {case['query']}")
            out, lat, tok = call_model(model_id, preprocessor, case["query"])
...
def run_streamlit_app():
        ...
        with st.spinner("🔄 Loading models and connecting to Knowledge Graph..."):
            try:
                ...
                st.session_state.preprocessor = FPLInputPreprocessor(
                    embedding_model_name=currentEmbedModel,
    ...
    st.markdown(
        "Ask questions about Fantasy Premier League players, teams, and statistics powered by Knowledge Graph!"
    )
    ...
    if run_button and query:
        with st.spinner("Processing query..."):
            try:
                response, tokens_used, processed, cypher_query = userQueryKG(
                    query,
                    model=selected_model,
                    preprocessor=st.session_state.preprocessor,
                    traceQuery=show_debug
                )
    ...
    if st.session_state.history:
        ...
        for item in reversed(st.session_state.history[-10:]):
            ...
            if show_debug:
                ...
                st.markdown(f"**Entities:** {item['entities']}")
                st.markdown(f"**Cypher:** {item['cypher']}")
            st.divider()

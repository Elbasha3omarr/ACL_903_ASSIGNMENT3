...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = ""  # Disable CUDA GPU
import torch
...
userQueryKG(queries, hf_model2)


# In[ ]:


userQueryKG(queries, hf_model3)
...
# =================================================
MODELS = {
    "deepseekV3": "HuggingFaceH4/zephyr-7b-beta",
    "deepseekV30324": "meta-llama/Llama-3.1-8B-Instruct",
    "Llama-3.3": "google/gemma-2-9b-it"
}
...
# MODEL CALL
# =================================================
def call_model(model, query, kg):
    start = time.time()
    r = client.chat_completion(
        model=model,
        messages=[
            {"role": "system", "content": "Use ONLY the FPL knowledge graph below."},
            {"role": "assistant", "content": kg},
...
for model_name, model_id in MODELS.items():
    ...
    for case in test_cases:
        out, lat, tok = call_model(model_id, case["query"], KG_TEXT)
        q = evaluate_quality(case["query"], out)

        rows.append({
            "model": model_name,   # ✅ FIXED: correct model name
            "query": case["query"],
            "latency": round(lat, 3),
            "tokens": tok,
